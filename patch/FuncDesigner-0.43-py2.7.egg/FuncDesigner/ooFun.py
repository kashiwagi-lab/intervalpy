...
# 20121118
#
# Override method of numpy.asfarray for interval.
# ref: ./math_interval.py
#
# -------- str ----------
...
from numpy import inf, copy, all, any, atleast_2d, zeros, dot, asarray, atleast_1d, \
ones, ndarray, where, array, nan, vstack, eye, array_equal, isscalar, log, hstack, sum as npSum, prod, nonzero,\
isnan, asscalar, zeros_like, ones_like, amin, amax, logical_and, logical_or, isinf, logical_not, logical_xor, flipud, \
tile, float64, searchsorted, int8, int16, int32, int64, isfinite, log2, string_, asanyarray
from math_interval import asfarray
# -------- end ----------
#
# auth.era
#
    ...
    def __div__(self, other):
        if isinstance(other, oofun):
            ...
            def aux_dx(x, y):
                ...
                #### str_20121118 ############################################################
                # Add x overwrapped 'afsarray' for dealing with differentiation of fraction
                #print "x: ", x, type(x)
                ...
                for key, val in elem_d.items():
                    if type(t1) == DiagonalType and type(val) == DiagonalType:
                        ...
                        * (val if isscalar(val) or prod(val.shape)>1 else asscalar(val) if type(val)==ndarray else val[0, 0])
                        ...
                        # 20121120
                        #
                        # For avoiding Error of scypy.sparse.cs(c/r)_matrix by interval-obj
                        #
                        # ---- str ---------------------------------------------------------------
                        #if not type(t1) == type(t2) ==  ndarray:
                        if hasattr(t1, 'Unsupport_scipy_sparse_cscmatrix') or hasattr(t2, 'Unsupport_scipy_sparse_csrmatrix'):
                            rr = dot(t1, t2)
                        elif not type(t1) == type(t2) ==  ndarray:
                        # ---- end ---------------------------------------------------------------
                        #
                        # Auth. era
                        ##########################################################################
